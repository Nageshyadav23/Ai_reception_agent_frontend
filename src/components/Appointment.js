...
const Appointment = () => {
  ...
  const fetchHistory = async () => {
    try {
      ...
      if (Array.isArray(data)) {
        const appointmentCalls = data
          .filter((call) => call.intent.toLowerCase() === "appointment")
          .sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp)); // recent first
        setHistory(appointmentCalls);
      }
    } catch (err) {
      console.error(err);
      setHistory([]);
  ...
  return (
    ...
    >
      ...
      >
        {history.length === 0 && <p>No Appointment calls found.</p>}
        {history.map((call) => (
          <div
            key={call.id}
            style={{
              ...
              borderRadius: "8px",
              backgroundColor:
                call.status === "completed" ? "#155724" : "#004085", // green = completed, blue = pending
              color: "#fff",
              transition: "all 0.4s",
